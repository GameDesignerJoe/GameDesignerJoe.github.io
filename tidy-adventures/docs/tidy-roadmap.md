...
- **Campaign is a job board**: a grid of faces, one tile per level, in the order you play them, off the same single progress integer. Faces are rationed — everything you've worked, the job you can start, and **exactly one job ahead, greyed**. Past that, blank silhouettes. So the board starts as a field of shapes with one person in it, and you always have precisely one tease. Arcs **interleave** — Marguerite pauses while the frat house happens and returns at the very end — and because the tiles are in play order you can see that happen instead of being told it. (First shipped grouped by client, with quotes and stage pips; that read as a column of text with the faces buried in it.)
- **👩‍🦰 Mom is the prologue.** Two levels, and they are your own room: "you want to make money this summer? Start with your own room." She teaches tap-and-place and what a home for a thing is, in the voice everyone already has in their head for it, and then hands you off — *"I told my friend Marguerite about you. Expect a call."* Marguerite's greyed face appears on the very next tile as she says it. A two-stage arc is legal precisely so a prologue can exist; `validate.js` allows 2, and wants 3–5 for anything with somewhere to go.
- **Then the real clients, twenty-two levels** (`levels.json` is append-only, because progress is an index into it). 👩 Marguerite clearing her grandmother's house is your first paying job and your first stranger; 🧑‍🎓 Delta Tau Chi runs the five-beat escalation; 👽 Zorb argues with the taxonomy.
