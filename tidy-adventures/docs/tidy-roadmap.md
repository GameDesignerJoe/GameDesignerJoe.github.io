...
- **Campaign is a job board**: a grid of faces, one tile per level, in the order you play them, off the same single progress integer. Everyone you haven't met yet is a blank silhouette, so the board is mostly shapes at the start and fills in with people as you work. Arcs **interleave** — Marguerite pauses while the frat house happens and returns at the very end — and because the tiles are in play order you can see that happen instead of being told it. (It was first shipped grouped by client, with quotes and stage pips; that read as a column of text with the faces buried in it.)
