...
#### 4.4.1 Pan Interaction Model
The map supports a Google Maps-style pan interaction model that allows unrestricted movement at any zoom level:

```typescript
// Helper to check if panning is possible
const canPan = useCallback(() => {
  // Allow panning whenever the image is loaded
...
Key features of the pan interaction:
1. Panning is always enabled when the image is loaded
2. No restrictions on pan distance
3. Smooth movement with 1.5x multiplier
4. Cursor feedback (grab/grabbing) indicates pannable state
5. Integer-aligned coordinates for crisp rendering

#### 4.4.2 Visual Feedback System

The interface provides consistent visual feedback through:

```typescript
// Container styling with cursor states
<div 
  ref={mapContainerRef} 
...
Control panel styling maintains consistent heights and spacing:
...
.map-controls, .grid-controls, .detail-info {
  height: 34px;
  display: flex;
  align-items: center;
